...
def pocket_atom_num_from_mol2(name):
    n = 0
...
def pocket_atom_num_from_pdb(name):
    n = 0
...
## function -- feature
def gen_feature(name):
    ligand = next(pybel.readfile('mol2', '%s/%s/%s_ligand.mol2' % (path, name, name)))
    ligand_coords, ligand_features = featurizer.get_features(ligand, molcode=1)
    if os.path.isfile('%s/%s/%s_pocket_new.mol2' % (path, name, name)):
        pocket = next(pybel.readfile('mol2' ,'%s/%s/%s_pocket_new.mol2' % (path, name, name)))
        pocket_coords, pocket_features = featurizer.get_features(pocket, molcode=-1)
        node_num = pocket_atom_num_from_mol2(name)
        try:
            assert node_num == len(pocket_coords)
            node_num = len(pocket_coords)
    else:
...
def pairwise_atomic_types(path, processed_dict):
...
def cons_lig_pock_graph_with_spatial_context(ligand, pocket, add_fea=2, theta=5, keep_pock=False, pocket_spatial=True):
    ...
    return size, lig_size, feas, edges, coords, atoms, atoms_raw, (lig_dist, lig_pock_dist, pock_dist)
...
def process_dataset(core_path, refined_path, dataset_name, output_path, cutoff):
    ...
    for name in tqdm(os.listdir(path)):
        if len(name) != 4:
            continue
        processed_dict[name] = gen_feature(name)
    ...
    ## graph construction
    refined_id, refined_data, refined_cofeat, refined_pk = [], [], [], []
    core_id, core_data, core_cofeat, core_pk = [], [], [], []
    ...
    for k, v in tqdm(data_dict.items()):
        ...
        if k in core_set_list:
            ...
            core_cofeat.append(cofeat)
            core_pk.append(pk)
            continue
        refined_id.append(k)
        refined_data.append(graph)
        refined_cofeat.append(cofeat)
    ...
    train_f = [refined_cofeat[i] for i in train_idxs]
    train_y = [refined_pk[i] for i in train_idxs]
    valid_g = [refined_data[i] for i in valid_idxs]
    valid_f = [refined_cofeat[i] for i in valid_idxs]
    valid_y = [refined_pk[i] for i in valid_idxs]
    train = (train_g, train_f, train_y)
    valid = (valid_g, valid_f, valid_y)

    with open(os.path.join(output_path, dataset_name + '_train.pickle'), 'wb') as f:
        pickle.dump(train, f)
    with open(os.path.join(output_path, dataset_name + '_valid.pickle'), 'wb') as f:
        pickle.dump(valid, f)
    with open(os.path.join(output_path, dataset_name + '_test.pickle'), 'wb') as f:
        pickle.dump(core, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_path', type=str, default='./data/PDBbind/')
    parser.add_argument('--dataset_name', type=str, default='refined')
